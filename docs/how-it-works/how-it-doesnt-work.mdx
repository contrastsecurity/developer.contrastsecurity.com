...
- [SQL injection](../learn-security/vulnerabilities/sqli) does not occur because someone used an ' character, it occurs when a user controls query input. We track the control and have full visibility of the query from the database API.
- [Command injection](../learn-security/vulnerabilities/command-injection) occurs when any user input reaches a command shell.
- [Path Traversal](../learn-security/vulnerabilities/path-traversal) occurs when users can control the file being accessed and is most often exploited when the directories go up.
